...
async function handleVoiceCommandPolish(
  ...
  const styleGuidance = polishStyleMode === "fullPolish"
    ...
    ? `SMART DICTATION POLISH — COHERENCE-FIRST (v15p3w, 2026-05-21). The user spoke this in a long-form toggle dictation; they were rambling, thinking aloud, and trusting polish to organize their stream of consciousness into text that makes complete sense.\n\n` +
      ...
      `  (B) ORPHANED-CORRECTION CLEANUP (NOT a generic filler hunt): when you consolidate a false start / self-correction and DROP the discarded alternative, ALSO remove any word left grammatically dangling by that removal — typically a contrast/connective word that pointed at the thing you just removed. Example: "send it to Lukas, actually send it to the whole team instead" → after dropping "to Lukas", the trailing "instead" is now orphaned (it referenced Lukas) → "send it to the whole team". Same for a dangling "rather", "actually", "no wait". CRITICAL — this ONLY applies to words orphaned BY YOUR rewrite. Do NOT hunt for and strip "instead"/"thing"/"basically"/etc. in general. If the word is part of Steph's intended final sentence, KEEP IT: "send it to Lukas instead of Calvin" keeps "instead" (real contrast, both kept); "the migration thing" keeps "thing" (that's how he said it); "let's basically rewrite it" keeps "basically" unless it's clearly a dropped false start. When in doubt, KEEP. The bar: only remove a word your OWN consolidation left pointing at nothing.\n` +
