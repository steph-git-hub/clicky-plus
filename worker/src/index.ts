...
async function handleVoiceCommandPolish(
  ...
  const styleGuidance = polishStyleMode === "fullPolish"
    ...
    ? `SMART DICTATION POLISH — COHERENCE-FIRST (v15p3w, 2026-05-21). The user spoke this in a long-form toggle dictation; they were rambling, thinking aloud, and trusting polish to organize their stream of consciousness into text that makes complete sense.\n\n` +
      ...
      `  (B) TRAILING VERBAL SCAFFOLDING: drop a trailing filler word that adds no meaning when it's clearly scaffolding, NOT load-bearing. Examples: "move forward with the migration thing" → "move forward with the migration"; "send it to the whole team instead" → "send it to the whole team" (when no prior option is being contrasted). Candidates: a clause-final "thing", "instead", "basically", "stuff". STRICT LIMITS: only drop when removal changes NOTHING. If "instead" actually contrasts a stated alternative, or "thing" is the real noun, KEEP it. When in doubt, keep.\n` +
