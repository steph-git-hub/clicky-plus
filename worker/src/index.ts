  ...
  const sessionConfig: Record<string, unknown> = {
    ...
    tools: overrides.tools ?? [
      ...
      },
      {
        ...
        description: "Ranked keyword search across Steph's Obsidian vault — matches individual terms (not just exact phrases) and ranks title hits highest. Use when he asks if he wrote anything about X, where his notes on Y are, what he captured about Z, OR which past Cowork session he worked on something in. Past Cowork sessions are archived under 'Chat Summaries/', named 'YYYY-MM-DD - Cowork - <topic>' — so a topic search finds the session. Returns top 15 notes by relevance with title, path, snippet, and score.",
        parameters: {
          type: "object",
          properties: {
            query: {
              type: "string",
              description: "Distinctive keywords for what you're after — prefer specific nouns (product/tool/feature names) over generic phrases. Terms are matched individually and ranked, so 'vyra video editor' finds a note even if its title only says 'Vyra'. If the first search misses, retry with different or simpler keywords rather than giving up.",
            },
          },
          required: ["query"],
        },
