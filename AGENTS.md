...
**Building from the terminal is fine** — agents can and should just run:

```bash
cd ~/clicky-plus && xcodebuild -project leanring-buddy.xcodeproj \
...
- Do not override `DEVELOPER_DIR` when building — a non-default Xcode changes the code signature and DOES invalidate TCC grants. Plain `xcodebuild` with the default toolchain is safe (verified 2026-08-05); see Build & Run.

## Coding Behavior (Karpathy principles)

Hygiene rules for any code Claude writes in this project. Bias toward caution over speed; for trivial edits, use judgment.
...
- Transform vague tasks into verifiable goals before starting.
  - "Add validation" → "Write a check for invalid inputs, then verify it triggers."
  - "Fix the bug" → "Reproduce the bug first, then confirm the fix removes the repro."
  - "Refactor X" → "Capture current behavior, then confirm it's unchanged after."
- For multi-step tasks, declare the success check up front and loop until it passes.
- Strong success criteria let Claude work independently. Weak criteria ("make it work") force constant clarification.
